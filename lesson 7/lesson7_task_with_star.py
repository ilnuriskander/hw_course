...
class RomanNums:
        ...
        def from_roman(self):
                ...
                total = 0
                for i, j in enumerate(self.numb):
                        if i + 1 < len(self.numb) and roman_numbers[self.numb[i]] < roman_numbers[self.numb[i + 1]]:
                                total -= roman_numbers[self.numb[i]]
                        else:
                                total += roman_numbers[self.numb[i]]
                return total

        def is_palindrome(self):
                pallindrom = self.from_roman()
                if list(str(pallindrom)) == list(str(pallindrom))[::-1]:
                        return True
                else:
                        return False
