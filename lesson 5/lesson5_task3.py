...
def everything_for_your_cat(cats_data):
    ...
    for j in range(len(cats_data)):
        human_fio = ' '.join(cats_data[j][-2:])  # преобразовываем фио в строку
        cats_name_age = ', '.join(map(str, cats_data[j][:-2]))  # преобразовываем имя и возраст кошки
        if human_fio in dict_cat_data:
            dict_cat_data[human_fio] = dict_cat_data.get(
                human_fio) + '; ' + cats_name_age
        else:
            dict_cat_data[human_fio] = cats_name_age
    our_str = ''.join([f'{key}: {value}\n' for key, value in dict_cat_data.items()])
    return our_str
